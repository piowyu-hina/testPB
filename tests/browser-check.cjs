...
const { Room } = require('../src/rules.js');
...
(async () => {
  ...
  const idle = () => page.waitForFunction(() => document.getElementById('game').getAttribute('aria-busy') === 'false');
  try {
    ...
    await tile(2, 1).click(); await idle();
    ...
    for (let steps = 0; steps < 60 && !model.finished; steps++) {
      ...
      model.hand.forEach((_, card) => {
        for (let y = 0; y < 5; y++) for (let x = 0; x < 5; x++) {
          const point = [x, y], preview = model.preview(card, point); if (!preview) continue;
          const score = (preview.removedId >= 0 ? 15 : 0) - preview.damage * 10 - ((x - 2) ** 2 + (y - 2) ** 2) * .1;
          if (!best || score > best.score) best = { card, point, score };
        }
      });
      if (!best) { await page.locator('#end-turn').click(); model.endTurn(); }
      else {
        await page.locator(`.card[data-index="${best.card}"]`).click();
        await tile(...best.point).click(); model.move(best.card, best.point);
    ...
    await page.mouse.click(15, 15); assert.equal(await page.locator('.card.selected').count(), 0);

    await page.setViewportSize({ width: 960, height: 540 });
    assert.equal(await page.evaluate(() => document.documentElement.scrollHeight <= innerHeight), true, 'Short desktop must fit without vertical scrolling');
    ...
    await phone.waitForFunction(() => document.getElementById('game').getAttribute('aria-busy') === 'false');
    assert.equal(await phone.locator('#actors .actor').count(), 4);
    await phone.close();
    assert.deepEqual(errors, []); assert.deepEqual(external, []);
    console.log('Browser checks passed: local file loading, PNG assets, preview, input lock, automatic turn, victory, defeat, replay, keyboard exclusion, desktop/mobile layout. No network requests or JS errors.');
