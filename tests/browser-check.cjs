...
(async () => {
  ...
  try {
    ...
    const hoveredTileBox = await tile(2, 2).boundingBox();
    const tooltipBox = await page.locator('#tile-info').boundingBox();
    assert.ok(Math.abs(tooltipBox.y + tooltipBox.height - hoveredTileBox.y) <= 16, 'tile tooltip should sit near the hovered tile');
    ...
    assert.ok((await phone.locator('.board-shell').boundingBox()).width >= 340);
    ...
    assert.deepEqual(await portrait.evaluate(() => [document.documentElement.scrollWidth, document.documentElement.scrollHeight]), [720, 1280]);
    await portrait.locator('#start-game').tap();
    assert.deepEqual(await portrait.evaluate(() => [document.documentElement.scrollWidth, document.documentElement.scrollHeight]), [720, 1280]);
    ...
    assert.ok(portraitBoard.width >= 630, 'portrait board leaves excessive side margins');
    assert.ok(portraitCard.y + portraitCard.height + 8 <= portraitEnd.y, 'portrait end turn overlaps cards');
    ...
    await landscape.locator('#start-game').tap();
    ...
    assert.ok(landscapeCard.y + landscapeCard.height + 4 <= landscapeEnd.y, 'landscape end turn overlaps cards');
