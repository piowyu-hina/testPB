...
- 村莊點「地下城」，查看森林遺跡後點「出發」進入戰鬥；公會尚未開放。角色立繪僅供展示，不提供放大或點擊操作。
...
- 莉娜使用飛刀、追影與迴旋斬。飛刀原地攻擊十字第一隻敵人並留刀；走進刀格獲得當回合限定、0 行動的小刀卡。場上可有多格刀，同格不疊加。全部手牌都不能使用才自動結束回合。粉色貓娘保留基本移動牌，詳見 `docs/RogueCards.md`。
- 刺芽團子攻擊上下左右相鄰格。古木守衛固定交替「揮枝」（正前方一格、擋住正面直線攻擊）與「扎根」（四個斜角相鄰格、解除正面防護）。未選牌時點怪物可查看下一次技能與朝向，並固定說明。
- 第三間的精英古木有 2 生命，每次命中造成 2 傷害。繞側背或趁扎根時攻擊；正面格擋仍消耗卡片及行動，不造成傷害。非致死攻擊會退回原格。
