...
- 三間依序為林間小徑、交錯伏擊、赤角守衛。前兩間清空敵人後，上方中央開啟出口。仍須選牌，依牌的走法移到出口，但不扣行動、不推進回合。用牌後立即補滿三張，並保留至少一張短步，避免手牌限制導致無法離開；直接點地板不會移動。
...
怪物沿用原有 imagegen 圖片；角色新增大頭與主畫面立繪。原始提示詞在 `docs/ArtPrompts.md`，冒險者提示詞在 `docs/CharacterArt.md`，莉娜提示詞在 `docs/LinaArt.md`。
