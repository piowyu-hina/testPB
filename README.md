...
- 僅滑鼠／觸控操作。主畫面的小頭像可切換冒險者與女性角色「莉娜」，選擇會記住。兩人目前共用規則；切換外觀不重置戰鬥進度。
...
怪物沿用原有 imagegen 圖片；角色新增大頭與主畫面立繪。原始提示詞在 `docs/ArtPrompts.md`，冒險者提示詞在 `docs/CharacterArt.md`，莉娜長髮版提示詞在 `docs/LinaLongHairArt.md`（初版留存於 `docs/LinaArt.md`）。
